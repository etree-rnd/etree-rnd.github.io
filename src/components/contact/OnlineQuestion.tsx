...
import styles from './OnlineQuestionMobile.module.css';
...
export default function OnlineQuestion() {
  ...
  return (
    <form ref={formRef} onSubmit={sendMail} className={styles.container}>
      ...
      <input type={'hidden'} name={'to_name'} value={'넥스트리온 관리자'} />
      <ul className={styles.formArea}>
        <li>
          ...
          <input
            type={'text'}
            name={'email'}
            placeholder={'이메일주소(ex. abc@abc.com)'}
            onInput={setFormData}
            className={styles.inputForm}
          />
        </li>
        <li>
          <textarea
            ...
            onInput={setFormData}
            placeholder={
              '서비스 또는 프로젝트 개발 의뢰 및 그밖에 궁금하신 점에 대해 간단한 문의 내용을 남기시면 빠른 시간 내에 연락드리겠습니다.'
            }
